...
function formatFit(c, options, format) {
	const r = options.r ? ` (R2=${options.r2.toFixed(2)})` : ``;
	...
	if (!m) return `y = ${errorPercents(b, s)}${r}`;
	switch (format) {
	case 'slope-intercept':
		return `y = ${errorPercents(b, s)} ${term(m)}x${r}`;
	default:
		return `y = ${errorPercents(y0, s)} ${term(m)}(x - ${x0})${r}`;
	}
}
