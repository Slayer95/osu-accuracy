...
function nodesu2ojsamaBeatMap2Params(metaData, mods, algo) {
	/* WARNING: Uses wild approximations */
	const params = {
		max_combo: metaData.maxCombo,
		nsliders: Math.floor(metaData.maxCombo / 3),
		ncircles: Math.floor(metaData.maxCombo / 3),
		nobjects: 2 * Math.floor(metaData.maxCombo / 3) + 1,
		base_ar: metaData.diffApproach,
		base_od: metaData.diffOverall,
		aim_stars: metaData.diffAim,
		speed_stars: metaData.diffSpeed,
		mods: mods,
		score_version: algo || 1,
	};

	return params;
}

function ojsamaDiff2Params(stars, algo) {
	const map = stars.map;
	const params = {
