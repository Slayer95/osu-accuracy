...
class ActionLog {
			...
			for (const [key, {count, startTime, endTime}] of queued) {
				try {
					if (!this.counters.has(key)) {
						...
						this.counters.set(key, this.counters.get(key) + count);
	...
	logN(key, nEvents, t0, forceSync) {
		if (this.parallelQueued && !forceSync) {
			...
			this.counters.set(key, this.counters.get(key) + nEvents);
	...
	getTime(key) {
		...
		if (!dt || key === 'osu_network_rtt') return null;
	...
	getAverageTime(key) {
		...
		return (dt[0] * 1E3 + dt[1] / 1E6) / count; // ms
	}
