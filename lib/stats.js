...
function lsq_regression(values) {
	...
	const fit_intercept = y_mean - fit_slope * x_mean;

	const error_variance = get_sum(values.map(([x, y]) => (y - x * fit_slope - fit_intercept) ** 2)) / get_count(values);
	const s = Math.sqrt(error_variance); // Standard error

	return {
		equation: [fit_slope, fit_intercept],
		element: [x_mean, y_mean],
		s: s,
		correlation: ['r', r],
		correlation2: ['R2', r ** 2],
	};
...
function theil_sen_sorted(values) {
	...
	for (let i = 0; i < count; i++) {
		for (let j = i + 1; j < count; j++) {
			theil_sen_sorted_next(collection, values[i], values[j], false);
			tau += Math.sign(values[j][0] - values[i][0]) * Math.sign(values[j][1] - values[i][1]);
		}
	}
	tau /= count * (count - 1) / 2;
	...
	const x_median = median_sorted(values.map(t => t[0]));
	...
	return {
		equation: [fit_slope, fit_intercept],
		element: [x_median, x_median * fit_slope + fit_intercept],
		s: s,
		correlation: ['t', tau],
		correlation2: ['T2', tau ** 2],
	};
}
...
function theil_sen_weighted_sorted(values) {
	...
	for (let i = 0; i < count; i++) {
		for (let j = i + 1; j < count; j++) {
			theil_sen_sorted_next(collection, values[i], values[j], true);
			tau += Math.sign(values[j][0] - values[i][0]) * Math.sign(values[j][1] - values[i][1]);
		}
	}
	tau /= count * (count - 1) / 2;
